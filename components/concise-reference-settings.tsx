...
export function MeasurementSettings({s,update,visible}:{s:M.State;update:Update;visible?:Set<string>}){
 ...
 return <details className="c-setting" open><summary>Удельная масса · г/м</summary><p className="c-help">Последнее подтверждённое значение по ПИД и кабелю предлагается в задании на извлечение. Оно фиксируется в отчёте.</p>
 {s.measurements.filter(m=>!visible||visible.has(m.id)).map(m=><div className="c-total-line" key={m.id}><span>ПИД{m.pid} · {M.material(s,m.material)}<small>{M.dateLabel(m.date)} · подтвердил {M.person(s,m.author)}</small></span><b>{M.fmt(M.measurementGrams(m))} г/м</b></div>)}
...
export function CrewSettings({s,update,visible}:{s:M.State;update:Update;visible?:Set<string>}){
 ...
 const crews=s.templates.filter(t=>!visible||t.members.some(e=>visible.has(t.id+'|'+e))).filter(t=>(!crewFilter||t.id===crewFilter)&&(!employeeFilter||t.members.includes(employeeFilter)));
 return <section className="c-form"><h3>Шаблоны бригад разделки</h3>{!visible&&<div className="c-grid-two"><Choice label="Проверить по сотруднику" value={employeeFilter} onChange={setEmployeeFilter} options={[["","Все сотрудники"],...s.employees.map(e=>[e.id,e.name] as [string,string])]}/><Choice label="Проверить по бригаде" value={crewFilter} onChange={setCrewFilter} options={[["","Все бригады"],...s.templates.map(t=>[t.id,t.name] as [string,string])]}/></div>}
 {crews.map(t=><div className="c-card" key={t.id}><button className="c-done-line" onClick={()=>{setTemplate(structuredClone(t));setSearch('')}}><span><b>{t.name}</b><small>{t.warehouse?M.warehouse(s,t.warehouse):'Склад не назначен'} · {t.active===false?'Отключена':'Действует'}</small></span><b>Изменить ›</b></button><p>{t.members.filter(id=>!visible||visible.has(t.id+'|'+id)).map(id=>M.person(s,id)+(s.employees.find(e=>e.id===id)?.active?'':' (не работает)')).join(', ')}</p></div>)}
