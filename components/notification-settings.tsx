...
export function NotificationSettings({s,update,visible}:{s:State;update:Update;visible?:Set<string>}){
 ...
 useEffect(()=>{let active=true;void fetch('/api/operational/status',{cache:'no-store'}).then(r=>r.json()).then(r=>{if(active&&typeof r.notificationsEnabled==='boolean')setDelivery(r.notificationsEnabled)}).catch(()=>{});return()=>{active=false}},[]);
 ...
 {delivery===false&&<p className="c-error" role="status">Отправка ботом отключена в конфигурации сервера. Настройки можно сохранить, но до включения отправки сообщения не придут.</p>}
 ...
 {edit.value.newTasks||edit.value.current||edit.value.overdue?<div className="c-inset"><strong>Пример сообщения</strong><p>Заполните задания.<br/>{(edit.value.current||edit.value.newTasks)&&<>За сегодня: 1. Срок — 21:00.<br/></>}{edit.value.overdue&&<>Просроченных: 2.<br/></>}Мои задания →</p><small>Количество рассчитывается при отправке; если подходящих заданий нет, сообщения не будет.</small></div>:<p className="c-help">Все уведомления этому сотруднику выключены.</p>}
