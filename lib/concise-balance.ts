...
export type BalanceQuery={scope:string;actor:string;owners?:string[];pids?:string[];material:string;unit:'tonnes'|'coils';from:string;to:string;column:BalanceColumn};
...
export function balanceCell(s:M.State,q:BalanceQuery){
 const raw=(q.unit==='coils'?coilMovements(s):M.movements(s)).filter(m=>m.material===q.material&&m.date<=q.to&&inBalanceQuery(s,q,m.warehouse));
