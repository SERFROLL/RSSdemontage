...
export function adjustmentReviews(docs:Doc[]){return docs.filter(a=>a.kind==="adjustment").filter(a=>docs.some(d=>d.id!==a.id&&["report","trip","opening","initialization","adjustment"].includes(d.kind)&&d.pid===a.pid&&d.date<=a.date&&(d.seq??0)>(a.data.checkedSeq??0)));}
