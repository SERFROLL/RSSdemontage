...
def prepare(source, history, today):
    ...
    return dict(batchId=batch['manifest']['batch_id'],state=state,identities=identities,manifest=batch['manifest'],expectedExtractionGrams=controls)

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--source',type=Path,required=True);p.add_argument('--history',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--today',required=True);a=p.parse_args()
    data=prepare(a.source,a.history,a.today);a.output.parent.mkdir(parents=True,exist_ok=True)
