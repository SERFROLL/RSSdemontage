...
check('Обороты внутри выбранной группы взаимно исключаются, раскрытия суммируются',()=>{for(const unit of ['tonnes','coils'])for(const column of ['before','plus','minus','after']){const q={scope:'owners',owners:['a','b'],actor:'boss',material:'c',unit,from:today,to:today,column};const r=B.balanceCell(s,q);assert.equal(r.value,M.round(r.contributions.reduce((n,v)=>n+v.quantity,0)));if(column==='minus')assert.equal(r.value,unit==='tonnes'?.01:0);}});
check('Материалы отбирают ПИД без выдуманного происхождения металлов',()=>{const st={...s,pids:[{id:'101',lengthM:100,cables:['c']},{id:'202',lengthM:100,cables:['other']}],materials:[...s.materials,{id:'other',name:'Другой',kind:'cable'}]};assert.deepEqual(SF.matchingPids(st,['other']),['202']);assert.deepEqual(SF.matchingPids(st,['c','other']),['101','202']);assert.deepEqual(SF.matchingPids(st,['copper']),[]);assert.deepEqual(SF.matchingPids(st,[]),[]);});
check('С оборотами и с остатком: ИЛИ, не чистое изменение',()=>{assert.equal(SF.visibleBalance({plus:2,minus:2,after:0},true,false),true);assert.equal(SF.visibleBalance({plus:0,minus:0,after:2},true,false),false);assert.equal(SF.visibleBalance({plus:0,minus:0,after:2},true,true),true);assert.equal(SF.visibleBalance({plus:0,minus:0,after:0},true,true),false);assert.equal(SF.visibleBalance({plus:0,minus:0,after:0},false,false),true);});
check('Прогресс ПИД и его документы следуют отбору склада',()=>{const before=JSON.stringify(s);const p=M.pidProgress(s,'101','2026-01-01',today,w=>w==='a101');assert.equal(p.dig,1000);assert.ok(p.docs.every(d=>d.assignment.warehouse==='a101'));assert.equal(M.pidProgress(s,'101','2026-01-01',today,w=>w==='b101').total,0);assert.equal(JSON.stringify(s),before);});
const PS=await import(`../${out}/pid-summary.mjs`);
const ps=structuredClone(legacyFixture);ps.pids=[{id:'101',lengthM:30000,cables:['c','other'],locality:'Город'}];ps.materials.push({id:'other',name:'Другой кабель',kind:'cable'});ps.documents=[];
const pw=(id,warehouse,work,qty,date=today,material='c')=>({id,kind:'work',taskId:id,date,assignment:{id,warehouse,material:work==='dig'?'':material,work,active:true,responsible:warehouse==='a101'?'a':'b',editors:['helper']},versions:[{version:1,qty,actor:'helper',at:date+'T12:00:00Z',reason:'',mode:'work',measure:{id:'m',date:'2026-01-01',kgPerM:2,confirmedBy:'a'},crew:[],rate:0,norm:[0,0,0]}]});
ps.documents=[pw('dig-a','a101','dig',20000),pw('dig-b','b101','dig',10000,'2026-08-31'),pw('extract-a','a101','extract',9000),pw('extract-other','a101','extract',9000,today,'other'),{id:'sent',kind:'transfer',date:today,from:'a101',to:'main',actor:'helper',items:[{material:'c',sent:10,received:9.9},{material:'other',sent:10,received:9.9}],weights:[],receivedAt:today,receiver:'b',reason:'Разница взвешивания'}];
check('ПИД: 20 км, извлечено 36 т, отправлено 20 т, на ПИД 16 т; автор не заменяет МОЛ',()=>{const before=JSON.stringify(ps),rows=PS.pidForemen(ps,'101','2026-09-01',today),r=rows.find(r=>r.employee==='a');assert.equal(rows.length,2);assert.ok(!rows.some(r=>r.employee==='helper'));assert.equal(r.dig,20000);assert.equal(B.balanceCell(ps,r.extracted).value,36);assert.equal(B.balanceCell(ps,r.sent).value,20);assert.equal(B.balanceCell(ps,r.stock).value,16);assert.deepEqual(B.balanceCell(ps,r.extracted).ids.sort(),['extract-a','extract-other']);assert.deepEqual(B.balanceCell(ps,r.sent).ids,['sent']);assert.equal(JSON.stringify(ps),before);});
check('ПИД: суммарные раскрытия сохраняют материалы, ПИД, МОЛ и точные вклады',()=>{const r=PS.pidForemen(ps,'101','2026-09-01',today,()=>true,['c'])[0];assert.equal(B.balanceCell(ps,r.extracted).value,18);assert.equal(B.balanceCell(ps,r.sent).value,10);assert.equal(B.balanceCell(ps,r.stock).value,8);for(const q of [r.extracted,r.sent,r.stock]){const v=B.balanceCell(ps,q);assert.equal(v.value,M.round(v.contributions.reduce((n,x)=>n+x.quantity,0)));assert.deepEqual(q.owners,['a']);assert.deepEqual(q.pids,['101']);}assert.equal(PS.pidForemen(ps,'101','2026-09-01',today,w=>w==='b101')[0].employee,'b');});
check('ПИД: остаток включает прошлые приходы и приёмку, извлечение их не включает',()=>{const st=structuredClone(ps);st.documents.push({id:'old',kind:'opening',date:'2026-08-01',warehouse:'a101',material:'c',qty:5,actor:'a'});const oldExtract=st.documents.find(d=>d.id==='extract-a');oldExtract.versions.unshift({...oldExtract.versions[0],qty:100});oldExtract.versions[1].version=2;const r=PS.pidForemen(st,'101','2026-09-01',today)[0];assert.equal(B.balanceCell(st,r.stock).value,21);assert.equal(B.balanceCell(st,r.extracted).value,36);const later=PS.pidForemen(st,'101','2026-09-25','2026-09-25')[0];assert.equal(later.dig,0);assert.equal(B.balanceCell(st,later.extracted).value,0);assert.equal(B.balanceCell(st,later.sent).value,0);assert.equal(B.balanceCell(st,later.stock).value,21);st.documents.push({id:'incoming',kind:'transfer',date:'2026-08-31',from:'b101',to:'a101',actor:'b',items:[{material:'c',sent:1,received:1}],weights:[],receivedAt:today,receiver:'a',reason:''});assert.equal(B.balanceCell(st,r.stock).value,22);assert.equal(B.balanceCell(st,r.extracted).value,36);assert.equal(B.balanceCell(st,r.sent).value,20);});
check('ПИД: ИЛИ, завершённость по всем МОЛ, отсутствие длины и превышение',()=>{const all=()=>true,from='2026-09-01';assert.equal(PS.visiblePid(ps,'101',from,today,all,false,false),true);assert.equal(PS.visiblePid(ps,'101',from,today,all,true,false),true);assert.equal(PS.visiblePid(ps,'101',from,today,all,false,true),false);assert.equal(PS.visiblePid(ps,'101',from,today,all,true,true),true);assert.equal(PS.visiblePid(ps,'101',from,today,w=>w==='a101',false,true),false);assert.equal(PS.visiblePid(ps,'101','2026-09-25','2026-09-25',all,true,true),false);for(const lengthM of [null,20000,40000])assert.equal(PS.visiblePid({...ps,pids:[{id:'101',lengthM}]},'101',from,today,all,false,true),true);});
...
check('Снятие всех функций и повтор генерации сохраняют старые задания',()=>{const changed=D.applyChanges(daily,[{key:'duties',rows:[{...daily.duties[0],functions:[]}]}],boss);assert.equal(changed.duties[0].active,false);assert.deepEqual(changed.dailyTasks,daily.dailyTasks);assert.deepEqual(changed.documents,daily.documents);assert.deepEqual(D.postings(changed),D.postings(daily));assert.equal(Daily.generateDaily(changed,today,10).dailyTasks.length,2);assert.equal(Daily.generateDaily(changed,'2026-09-25',10).dailyTasks.length,3)});
check('Функции определяют активацию, прежние выключенные назначения не оживают',()=>{const d={...daily.duties[0],active:false};const legacy={...daily,duties:[d]};assert.deepEqual(Daily.effectiveFunctions(d),[]);assert.equal(Daily.generateDaily(legacy,'2026-09-25',10).dailyTasks.length,2);const next=Daily.saveDuty(legacy,{...d,functions:['dig']});assert.equal(next.duties[0].active,true);assert.deepEqual(Daily.generateDaily(next,'2026-09-25',10).dailyTasks.at(-1).functions,['dig']);assert.throws(()=>D.applyChanges(daily,[{key:'duties',rows:[{...d,functions:[]}]}],helper),/администратор/);});
